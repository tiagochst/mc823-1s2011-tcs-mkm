...
Um sistema distribuído, segundo Andrew Tanenbaum, é uma "coleção de computadores independentes que se apresenta ao usuário como um sistema único e consistente". Para que isso seja possível, diversos computadores estão interligados por uma rede de computadores, através da qual compartilham entre si objetos (como arquivos, informações, processamento, etc) e são responsáveis por manter uma consistência nesses objetos. 
Portanto, hoje, um servidor não é apenas um computador, mas sim vários computadores em locais diferentes que aparentam ser, para um usuário, um único sistema.
